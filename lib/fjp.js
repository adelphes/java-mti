...
class FJP {
    ...
    static java_to_json(source) {
        let lastdecl;

        function error(e) {
            console.log('last decl...');
        ...
        function constant_expression(o) {
            if (o.source.match(/^\(/)) {
                o.source = o.source.slice(1); stripwsc(o);
                const ce = constant_expression(o);
                if (!o.source.match(/^\)/)) error();
                o.source = o.source.slice(1); stripwsc(o);
                return '('+ce+')';
            }
            // for now, just allow single literals or identifiers with simple operators
            const re = /^[~!+-]*([a-zA-Z_][a-zA-Z0-9_]*(\s*\.\s*[a-zA-Z_][a-zA-Z0-9_]*)*|0x[0-9a-fA-F_]+[dDfFlL]?|[0-9_]+e[+-]?[0-9_]+[dDfFlL]?|[0-9_]+(\.[0-9]*)?[dDfFlL]?|\.[0-9][0-9_]*[dDfFlL]?|'(\\u[0-9a-fA-F]{4}|\\.|.)'|"([^\\\n"]*\\.)*[^\n"]*")/;
            const m = o.source.match(re);
            if (!m) error();
            o.source = o.source.slice(m[0].length);
            stripwsc(o);
            ...
            if (opm) {
                ...
                stripwsc(o);
                return m[0] + ' ' + opm[0] + ' ' + constant_expression(o);
            }
            return m[0];
        }
        function ident(o) {
