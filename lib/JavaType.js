...
class JavaType extends Declaration {
     ...
     * @param {string} docs 
     */
    constructor(short_signature, typeKind, mods, docs) {
        super(mods, docs);
        this.typeKind = typeKind;
        this.shortSignature = short_signature;
    }
    ...
    asClass() {
        return this.resolveType(`java/lang/Class<L${this.shortSignature};>`)
    }
...
class PrimitveJavaType extends JavaType {
    ...
    constructor(sig, name) {
        super(sig, "primitive", 0, `primitive ${name}`);
        this._signature = sig;
...
class ArrayJavaType extends JavaType {
     ...
     */
    constructor(base, arrdims, arrprefix = '['.repeat(arrdims)) {
        super(`${arrprefix}${base.shortSignature}`, "array", 0, '');
        this.base = base;
        this.arrdims = arrdims;
        this._arrprefix = arrprefix;
...
class CEIJavaType extends JavaType {
    ...
    get rawTypeSignature() {
        return `L${this.shortSignature};`;
    }

    get typeSignature() {
        // @ts-ignore
        const tvsignatures = this.typevars.map(t => t instanceof TypeArgument ? t.type.typeSignature : '').join('');
        return `L${this.shortSignature}${tvsignatures && `<${tvsignatures}>`};`;
    }
}
...
class UnresolvedType extends CEIJavaType {
    constructor(signature) {
        super(signature, 'class', 0, '');
        this.simpleTypeName = signature;
    }
}
...
class CompiledJavaType extends CEIJavaType {
    ...
    constructor(decoded, typemap) {
        ...
        this._name = new RawTypeName(decoded.thisclass);
        super.packageName = this._name.packageName;
        super.simpleTypeName = this._name.simpleTypeName;
        this._fullyDottedTypeName = this._name.fullyDottedRawName;
        ...
        if (sigattr) {
            ...
            super.typevars = typevars;
            this._supers = superclasses;
        }
        else {
            /** @type {string[]} */
            this._supers = decoded.superclass ? [decoded.superclass] : [];
        }
    ...
    get fullyDottedTypeName() {
        return this._fullyDottedTypeName;
    }
    ...
    specialise(types) {
        ...
        const ta_label = `<${typeargs.map(t => t.label).join(',')}>`;
        specialised_type.typevars = typeargs;
        specialised_type.simpleTypeName = this._name.simpleTypeName + ta_label;
        specialised_type._fullyDottedTypeName = this._name.fullyDottedRawName + ta_label;
        ...
        if (!type) {
            ...
            if (generictype) {
                ...
                if (rawtype) {
                    ...
                    // todo - specialise the generic type
